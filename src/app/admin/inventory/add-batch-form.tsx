...
export function AddBatchForm({ products }: { products: Product[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.product_id || !form.quantity || !form.expiry_date) {
      toast.error('Produk, kuantiti dan tarikh tamat wajib diisi')
      return
    }
    ...
    if (error) {
      ...
      toast.success('Batch stok ditambah')
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">Harga Kos (RM)</label>
          <input
            name="cost_price"
            type="number"
            step="0.01"
      ...
      <div>
        <label className="block text-xs font-medium text-gray-600 mb-1">Tarikh Tamat Tempoh *</label>
        <input
          name="expiry_date"
          type="date"
