...
import { StockSemasa } from './stok-semasa'
    ...
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="flex flex-wrap gap-2">
        <div className="flex items-center gap-2 bg-white border border-gray-100 rounded-xl px-4 py-2.5 shadow-sm">
          ...
          <span className="text-sm font-medium text-gray-700">{products.length} product active</span>
        </div>
        {outOfStock > 0 && (
          <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-4 py-2.5">
            <TrendingDown className="h-4 w-4 text-red-500" />
            <span className="text-sm font-semibold text-red-700">{outOfStock} kehabisan stock</span>
          </div>
        )}
        {lowStock > 0 && (
          <div className="flex items-center gap-2 bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-2.5">
            <AlertTriangle className="h-4 w-4 text-yellow-500" />
            <span className="text-sm font-semibold text-yellow-700">{lowStock} stock rendah</span>
          </div>
        )}
            ...
            <div className="space-y-1">
              {expiringSoon.map((b: any) => (
                <p key={b.id} className="text-xs text-orange-700">
                  ...
                  {new Date(b.expiry_date).toLocaleDateString('en-MY', { day: 'numeric', month: 'short', year: 'numeric' })}
                </p>
              ))}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sticky top-6">
            <h2 className="font-bold text-gray-900 mb-4">Add Batch Stock</h2>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <StockSemasa products={products as any} stockMap={stockMap} />

          {/* Batch list */}
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-3.5 border-b border-gray-100">
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-bold text-gray-900">Batch Stock</h2>
              ...
              <form method="GET" action="/admin/inventory" className="relative">
                ...
                <input
                  name="bq"
                  defaultValue={bq}
                  placeholder="Search batch mengikut nama product..."
            ...
            <table className="w-full text-sm min-w-[500px]">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  ...
                  <th className="px-5 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wide">Tamat Period</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {batches.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-5 py-10 text-center text-gray-400">No batch lagi</td>
                  </tr>
                ) : (
                  batches.map((b: any) => {
                    ...
                    return (
                      <tr key={b.id} className={`transition-colors ${exp.rowCls || 'hover:bg-gray-50'}`}>
                        ...
                        <td className="px-5 py-3 text-center text-xs text-gray-500">
                          {new Date(b.batch_date).toLocaleDateString('en-MY', { day: 'numeric', month: 'short' })}
                        </td>
