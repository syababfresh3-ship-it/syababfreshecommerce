...
async function getLoyaltyData() {
  ...
  const [profileRes, tiersRes, txRes] = await Promise.all([
    supabase
      .from('profiles')
      .select('full_name, total_points, total_spend, referral_code, loyalty_tiers(name, min_spend, multiplier)')
      .eq('id', user.id)
      .single(),
    supabase
      .from('loyalty_tiers')
            ...
            <div className="space-y-3">
              ...
              <div className="bg-white rounded-2xl shadow-[0_2px_14px_rgba(0,0,0,0.07)] p-4 flex gap-6">
                ...
                <div className="text-center flex-1">
                  ...
                  <p className="text-[10px] text-gray-400">bila rakan buat order pertama</p>
                </div>
              </div>
            </div>
          )}
